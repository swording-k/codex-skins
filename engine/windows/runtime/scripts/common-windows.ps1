...
function Invoke-DreamSkinNative {
  ...
  try {
    # Codex Theme Creator ships Electron rather than a second Node archive.
    # Electron exposes the Node CLI when this is present in its child process.
    if ((Split-Path -Leaf $FilePath) -ieq 'Codex Theme Creator.exe') {
...
function Get-DreamSkinNodeRuntime {
  ...
  if ($env:CODEX_THEME_NODE) {
    return Get-DreamSkinValidatedNodeRuntime -Path $env:CODEX_THEME_NODE -MinimumMajor $MinimumMajor
  }
