...
通过一句话或一张参考图，让 Codex 为 Codex Desktop 创作、安装并验证一套完整主题；再用本地 App 像换壁纸一样管理和切换。
...
2. 在自己电脑上安装本项目。
3. 启动 Codex Theme Creator App。
4. 在 App 里选择、保存、微调、启用主题。
5. 想创作新主题时，回到 Codex，说出想法或发参考图。
...
Windows 用户可以先用 App 管理主题包：

```powershell
git clone https://github.com/swording-k/codex-theme-creator.git
cd codex-theme-creator
powershell -ExecutionPolicy Bypass -File .\scripts\start-theme-app.ps1
```

Windows 当前还不能一键注入 Codex，App 会显示“可保存主题，暂不支持一键切换”。

如果暂时不想装桌面壳，也可以运行开发用控制台：

```bash
./scripts/start-theme-studio.sh
```

这时浏览器里的 `http://127.0.0.1:56938/` 是用户自己电脑上的本地控制台，不是我们的网站。
...
App 不负责凭空替代 Codex 创作主题。它做三件更适合普通用户的事：

- 管理：显示公开预设和自己创作过的主题；
- 微调：改强调色、背景模糊、背景压暗、新聊天透明度、任务页透明度；
- 切换：把选中的主题应用到 Codex。

创作仍然交给 Codex：你把想法或参考图发给 Codex，Codex 用本项目的 Skill 生成完整主题包。主题包生成后会出现在 App 的主题列表里。

启动桌面 App 后，它会常驻为一个小入口：

- macOS：右上角菜单栏图标；
- Windows：右下角系统托盘图标。

点击这个小图标可以打开主题管理器、刷新主题列表，并快速切换已经保存到本机的主题。
...
App 里也会直接显示当前机器的主题库路径。对普通用户来说，最推荐的方式仍然是让 Codex 按 Skill 创作并安装；手动导入只适合已经拿到完整主题包的人。

### 调节会不会立刻生效

会。Theme Studio 里的右侧滑杆会先生成一套临时预览主题，并实时注入到正在运行的 Codex Desktop。你可以边拖边看背景模糊、压暗、强调色和面板透明度的变化。
...
实时预览使用固定的临时主题 ID，不会把每一次拖动都堆进你的主题列表。真正想长期保留时，再保存成“我的主题”。
...
2. 想微调：在 Theme Studio 或 App 里调模糊、压暗、透明度和强调色。
